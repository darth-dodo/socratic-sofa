...
def export_dialogue_markdown(
    topic: str, proposition: str, opposition: str, judgment: str, timestamp: str = None
) -> str:
    """
    Export the dialogue as a formatted Markdown document.

    Args:
        topic: The philosophical topic
        proposition: First line of inquiry
        opposition: Alternative line of inquiry
        judgment: Dialectic evaluation
    ...
    return f"""# Socratic Dialogue: {topic}
...
*Generated by [Socratic Sofa](https://github.com/darth-dodo/socratic-sofa) using the Socratic method.*
"""


def export_dialogue_json(
    topic: str, proposition: str, opposition: str, judgment: str, timestamp: str = None
) -> str:
    """
    Export the dialogue as a JSON document.
    ...
    data = {
        "metadata": {
            "generator": "Socratic Sofa",
            "timestamp": timestamp,
            "version": "1.0",
        },
        "dialogue": {
            "topic": topic,
            "first_inquiry": proposition,
            "alternative_inquiry": opposition,
            "evaluation": judgment,
        },
    }
...
with gr.Blocks(title="Socratic Sofa - Philosophical Dialogue") as demo:
    ...
    with gr.Row():
        with gr.Column():
            gr.Markdown("### 📥 Export Dialogue")
            with gr.Row():
                export_md_button = gr.Button("📄 Download Markdown", size="sm")
                export_json_button = gr.Button("📊 Download JSON", size="sm")
            export_file = gr.File(label="Download", visible=False)

    # Export handler functions
    def handle_export_markdown(topic, proposition, opposition, judgment):
        """Generate and return markdown export file."""
        if not topic or topic.startswith("⏳") or topic.startswith("❌"):
            return gr.update(visible=False)

        content = export_dialogue_markdown(topic, proposition, opposition, judgment)
        # Create a temporary file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"socratic_dialogue_{timestamp}.md"
        filepath = Path("/tmp") / filename
        filepath.write_text(content, encoding="utf-8")
        return gr.update(value=str(filepath), visible=True)

    def handle_export_json(topic, proposition, opposition, judgment):
        """Generate and return JSON export file."""
        if not topic or topic.startswith("⏳") or topic.startswith("❌"):
            return gr.update(visible=False)

        content = export_dialogue_json(topic, proposition, opposition, judgment)
        # Create a temporary file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"socratic_dialogue_{timestamp}.json"
        filepath = Path("/tmp") / filename
        filepath.write_text(content, encoding="utf-8")
        return gr.update(value=str(filepath), visible=True)
    ...
    export_md_button.click(
        ...
        inputs=[topic_output, proposition_output, opposition_output, judgment_output],
        outputs=[export_file],
    )
